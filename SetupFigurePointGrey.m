...
%% UI ImagePanel 
    ...
    Xin.UI.FigPGC(N).hPanelImage = uipanel(...
        ...
        Xin.UI.FigPGC(N).hAxesImage = axes(...
            ...
            'CLimMode',     'Manual',...
            'ALimMode',     'Manual');
        Xin.UI.FigPGC(N).hImage = image(Xin.D.Sys.PointGreyCam(N).DispImg3,...
            'parent',       Xin.UI.FigPGC(N).hAxesImage);        
        
        Xin.UI.FigPGC(N).hText = text(0, 0, {' 0', ' 0', ' 0'},...
            'parent',       Xin.UI.FigPGC(N).hAxesImage,...
            'Color',        [1 1 1],...
            'VerticalAlignment',    'Top',...
            'HorizontalAlignment',  'Left');  
        Xin.UI.FigPGC(N).hAxesImage.NextPlot =  'add';
        Xin.UI.FigPGC(N).hPlotV = plot(...
            (1+Xin.D.Sys.PointGreyCam(N).ZoomWidth)/2*[1 1],...
            [1 Xin.D.Sys.PointGreyCam(N).ZoomHeight],           '--',...
            'parent',       Xin.UI.FigPGC(N).hAxesImage,...
            'visible',      'off'); 
        Xin.UI.FigPGC(N).hPlotH = plot(...
            [1 Xin.D.Sys.PointGreyCam(N).ZoomWidth],...
            (1+Xin.D.Sys.PointGreyCam(N).ZoomHeight)/2*[1 1],	'--',...
            'parent',       Xin.UI.FigPGC(N).hAxesImage,...
            'visible',      'off'); 
...
S.PnltCurrent.row = 2;      S.PnltCurrent.column = 2;          
    WP.name = 'Exp RefImage';
        ...
        Xin.UI.FigPGC(N).CP.hExp_RefCoord_Momentary =	Xin.UI.FigPGC(N).Panelette{WP.row,WP.column}.hMomentary{2}; 
        set(Xin.UI.FigPGC(N).CP.hExp_RefImage_Momentary,	'UserData',     N);
        set(Xin.UI.FigPGC(N).CP.hExp_RefCoord_Momentary,	'UserData',     N);
        clear WP;       
